    ...
    <div class="w-[90%] mt-6">
        ...
        switch ($seccion) {
            ...
            case 'juegos':
                ...
                echo '<form method="POST" enctype="multipart/form-data" class="mb-6 flex flex-col gap-8 bg-gray-900 pt-12 pb-12 px-0 sm:p-4 sm:rounded-lg rounded-none shadow-sm w-full">';
                echo '<input type="hidden" name="nuevo_juego" value="1">';
                echo '<label class="flex items-center gap-2 mb-2"><input type="checkbox" name="popular" value="1" class="accent-cyan-600"> Marcar como popular</label>';
                echo '<input type="text" name="nombre" placeholder="Nombre del juego" class="block w-full text-2xl px-6 py-4 rounded-none sm:rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400" required />';
                echo '<textarea name="descripcion" placeholder="Descripción" class="block w-full text-xl px-6 py-4 rounded-none sm:rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400"></textarea>';
                echo '<input type="number" step="0.01" name="precio" placeholder="Precio" class="block w-full text-xl px-6 py-4 rounded-none sm:rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400" required />';
                echo '<label class="block mb-2">Imagen principal:<input type="file" name="imagen" accept="image/*" class="block w-full mt-2" /></label>';
                echo '<label class="block mb-2">Imagen común para paquetes:<input type="file" name="imagen_paquete" accept="image/*" class="block w-full mt-2" /></label>';
                echo '<select name="moneda" class="block w-full text-xl px-6 py-4 rounded-none sm:rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400 mb-2">';
                ...
                echo '</select>';
                echo '<label class="block mb-2">Seleccionar características existentes:';
                echo '<select name="caracteristicas[]" multiple class="block w-full text-xl px-6 py-4 rounded-none sm:rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400">';
                echo '<option value="Entrega Inmediata">Entrega Inmediata</option>';
                echo '<option value="Global">Global</option>';
                echo '</select></label>';
                echo '<input type="text" name="nueva_caracteristica" placeholder="Nueva característica" class="block w-full text-xl px-6 py-4 rounded-none sm:rounded bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400 mb-2" />';
                echo '<button type="submit" class="w-full bg-cyan-600 hover:bg-cyan-700 text-white text-2xl px-6 py-6 rounded-none sm:rounded font-semibold transition">Agregar juego</button>';
                echo '</form>';
                if (count($juegos) === 0) {
